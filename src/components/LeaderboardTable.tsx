...
export function LeaderboardTable({ leaderboard, selectedMatch }: LeaderboardTableProps) {
  const isGeneralView = selectedMatch === null
    ...
    <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:col-span-2">
      <div className="border-b border-slate-100 bg-white p-6">
        ...
        <p className="mt-1 text-sm text-slate-500">
          {isGeneralView ? (
            <span className="font-semibold text-blue-600">Clasificación general</span>
          ) : (
            <>
              Mostrando predicciones para:{' '}
              <span className="font-semibold text-blue-600">{selectedMatch.label}</span>
            </>
          )}
        </p>
      </div>
      ...
      {isGeneralView ? (
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="bg-slate-50 text-sm uppercase tracking-wider text-slate-600">
                <th className="w-2/3 p-4 font-semibold">Participante</th>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px] border-collapse text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {leaderboard.map((user, index) => {
                const selectedPoints = user.selectedItemPoints
                const isResolved = selectedPoints !== null
                const isPerfectMatch = isResolved && selectedPoints >= 3
                const isPartialMatch = isResolved && selectedPoints > 0 && selectedPoints < 3
